...
void assert_free_block_count(size_t size, int count) {
  ...
  for (int i = 0; i < NUM_FREE_LISTS; i++) {
    ...
    while (bp != &sf_free_list_heads[i]) {
      if (size == 0 || size == (bp->header & ~0x7)) cnt++;
  ...
  if (size == 0) {
    ...
    cr_assert_eq(cnt, count,
                 "Wrong number of free blocks of size %ld (exp=%d, found=%d)",
                 size, count, cnt);
  }
}
...
void assert_free_list_size(int index, int size) {
  ...
  cr_assert_eq(
      cnt, size,
      "Free list %d has wrong number of free blocks (exp=%d, found=%d)", index,
      size, cnt);
}
...
void assert_quick_list_block_count(size_t size, int count) {
  ...
  if (size == 0) {
    cr_assert_eq(cnt, count,
                 "Wrong number of quick list blocks (exp=%d, found=%d)", count,
                 cnt);
  } else {
    cr_assert_eq(
        cnt, count,
        "Wrong number of quick list blocks of size %ld (exp=%d, found=%d)",
        size, count, cnt);
  }
}
...
Test(sfmm_basecode_suite, realloc_larger_block, .timeout = TEST_TIMEOUT) {
  ...
  cr_assert((bp->header & ~0x7) == 88,
            "Realloc'ed block size not what was expected!");
...
Test(sfmm_basecode_suite, realloc_smaller_block_splinter,
...
Test(sfmm_basecode_suite, realloc_smaller_block_free_block,
     .timeout = TEST_TIMEOUT) {
  ...
  sf_block *bp = (sf_block *)((char *)y - sizeof(sf_header));
  cr_assert(bp->header & THIS_BLOCK_ALLOCATED, "Allocated bit is not set!");
  cr_assert((bp->header & ~0x7) == 32,
            "Realloc'ed block size not what was expected!");

  // After realloc'ing x, we can return a block of size
  // ADJUSTED_BLOCK_SIZE(sz_x) - ADJUSTED_BLOCK_SIZE(sz_y) to the freelist. This
  // block will go into the main freelist and be coalesced. Note that we don't
  // put split blocks into the quick lists because their sizes are not sizes
  // that were requested by the client, so they are not very likely to satisfy a
  // new request.
  assert_quick_list_block_count(0, 0);
  assert_free_block_count(0, 1);
  assert_free_block_count(4024, 1);
